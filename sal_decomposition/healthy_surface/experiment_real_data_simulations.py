...
if __name__ == '__main__':
    ...
    lr = 5e-3
    loss='negentropy'
    opt = 'search_fit'  # 'fit', 'search_fit', 'search'
    Nt = 50
    nepochs = 250
    ...
    for sub_idx in [0,1,2]: 
        ...
        for mvc in [25, 50]:
            for ses_idx in [0,1,2]:
                ...
                ############ FOR RERUNNING PURPOSES ONLY ############
                if (sub_idx != 2) or (mvc != 50) or (ses_idx not in [0, 1]):
                    continue
                #####################################################
                ...
                try:
                    for trans_idx in range(Nt): # Nt random transformations
                        ...
                        if opt == 'fit':
                            sources, losses = utils.search_fit_sda(emg_grid_test.to(torch.float32), sda.to(device), base_loss, npoints=0, nepochs=nepochs, batch_size=batch_size, boundaries=bounds, lr=lr, device=device, loss=loss, plot=0, frozen_sep_mat=True)
                        elif opt == 'search_fit':
                            sources, losses = utils.search_fit_sda(emg_grid_test.to(torch.float32), sda.to(device), base_loss, npoints=int(1.5*nepochs), nepochs=nepochs//2, batch_size=batch_size, boundaries=bounds, lr=lr, device=device, loss=loss, plot=0, frozen_sep_mat=True)
                        elif opt == 'search': # search only
                            sources, losses = utils.search_fit_sda(emg_grid_test.to(torch.float32), sda.to(device), base_loss, npoints=3*nepochs, nepochs=0, batch_size=batch_size, boundaries=bounds, lr=lr, device=device, loss=loss, plot=0, frozen_sep_mat=True)
                        else:
                        ...
                        run = wandb.init(
                            ...
                            project='sda-real-data-simulations2',
                            name=f'sub_{sub_idx+1}_ses_{ses_idx+1}_mvc_{mvc}',
                            config={'Subject': sub_idx+1, 'Session':ses_idx+1, 'MVC': mvc, 'opt': opt},
                            # mode='disabled'
                        )
                    ...
                    print('ERROR OCCURRED DURING PROCESSING OF THE GIVEN TRANSFORMATION! SKIPPING TO NEXT ONE...')
                    continue