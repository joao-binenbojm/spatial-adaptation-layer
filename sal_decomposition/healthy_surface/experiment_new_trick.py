                ...
                for R in [16]:#[8, 16, 32]:
                    for explained_var in [1-1e-2, 1-1e-3, 1-1e-4, 1-1e-6]:
                        ...
                        for trans_idx in range(30): # thirty random transformations
                            ...
                            ########################### TESTING WHETHER WE CAN ZERO OUT CHANNELS THAT ARE REMOVED OT REDUCE EFFECTS OF ZEROED CHANNELS ########################################
                            ...
                            # # RECOMPUTING EVERYTHING AFTER CROPPING!!
                            # emg_grid_test = emg_grid_test[:,:,3:, 1:]
                            # emg_grid = emg_grid[:,:,:-1, 1:]
                            # H, W = H-1, W-1
                            # sda.sal.input_shape = (H, W)
                            # sda.grid_shape = (H, W)
                            # sda.nchans = H*W
