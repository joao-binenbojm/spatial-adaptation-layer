...
from networks_utils import median_pool_2d, wrap_grid_horizontally
...
def get_grid_distance(grid_shape, true_params, learned_params, IED=1, circular=False):
    ...
    original_grid[:,:,:,0] = W*(1 + original_grid[:,:,:,0])/2
    original_grid[:,:,:,1] = H*(1 + original_grid[:,:,:,1])/2
    ...
    net_theta = theta_true @ theta_learned # apply inverse transformation and get resulting grid
    ...
    final_grid[:,:,:,0] = W*(1 + final_grid[:,:,:,0])/2
    final_grid[:,:,:,1] = H*(1 + final_grid[:,:,:,1])/2
...
#     if circular: true_grid = wrap_grid_horizontally(true_grid, H, W)
...
#     if circular: learned_grid = wrap_grid_horizontally(learned_grid, H, W)
        ...
        for session in tqdm(data['sessions']):
            ...
            for test_idx in sample_reps:
                ...
                for param_name in exp['adaptation_params'].keys():
                    ...
                    else: learned_params[param_name].append(scaling*param.item())
