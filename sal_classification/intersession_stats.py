...
def create_intersession_cond_col(row):
    if row['p_input'] and row['real_baseline']:
        return 'BN+Drpt.'
    elif row['p_input'] and row['learnable_baseline']:
        return 'LBN+Drpt.'
    elif not row['p_input'] and row['real_baseline']:
        return 'BN'
    elif not row['p_input'] and row['learnable_baseline']:
        return 'LBN'
...
exps = pd.read_csv('intersession.csv').sort_values(by='run-name')
...
for runname in runnames:
    print(runname)
    df_list.append(pd.read_csv(os.path.join('intersession', f'{runname}.csv')))
...
df_tot = df_tot[~((df_tot['adaptation'] == 'fine-tuning') & (df_tot['real_baseline'] == False) & (df_tot['p_input'] == 0.5))] .reset_index(drop=True)# remove conditions we exclude for fine-tuning
df_tot.loc[df_tot['adaptation'] == 'fine-tuning', 'conditions'] = 'fine-tuning'
...
conditions = ['no-adaptation', 'BN', 'BN+Drpt.', 'LBN', 'LBN+Drpt.', 'fine-tuning']
...
# Plotting
colours = ['midnightblue', 'slateblue', 'cornflowerblue', 'mediumpurple', 'lightsteelblue', 'lavender']
...
plt.ylim([0.0, 1.0])
# plt.show()
plt.savefig(f'C:\\Users\\Joao\\Desktop\\inter_{dataset}.png')