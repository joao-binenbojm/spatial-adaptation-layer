...
if __name__ == '__main__':
    ...
    for sub in tqdm(range(1, 9)):
        ...
        subs.append(sub)
        # X_train, y_train, X_test, y_test = intrasession_load(DIR='../datasets/capgmyo/dba', sub=sub, wlen=1, stride=1)
        train = CapgMyoSubjectLoader(path=DIR, db='dbb', sub=(2*sub - 1), window=1, stride=1)
        test = CapgMyoSubjectLoader(path=DIR, db='dbb', sub=2*sub, window=1, stride=1)
